...
export default function CustomNavbar({ variant, onSwitchView }: CustomNavbarProps) {
  ...
  return (
    <div className="relative w-full mt-[10px]">
      ...
      <div className="absolute right-0 top-0 left-[60%] h-full z-10 rounded-md hidden md:block"
        style={{
          clipPath: "polygon(100% 0, 100% 100%, 30% 100%, 0 0)",
        }}
      />

      <nav className={`absolute right-0 top-0 ${currentStyle.container} py-3 px-6 w-[65%] z-20 rounded-l-md hidden md:block`}
        style={{
      ...
      >
        <div className="flex justify-between items-center ml-20">
          ...
          <button
            ...
            className={`${currentStyle.button} px-4 py-2 rounded-md font-medium flex items-center relative`}
            style={{
              clipPath: "polygon(100% 0, 100% 100%, 10% 100%, 0 0)",
            }}
          >
            <span className="mr-8">Book a Demo</span>
            <span className={`absolute right-2 top-1/2 transform -translate-y-1/2 ${currentStyle.buttonIcon} rounded-full p-1`}>
